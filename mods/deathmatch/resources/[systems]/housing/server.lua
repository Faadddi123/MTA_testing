...
function reloadHouses()
    ...
    for _, row in ipairs(rows) do
        ...
        if ex == 0 and ey == 0 and not hasOwner then
            -- Safely ignore this ghost property frame
        else
            local h = {
                id = tonumber(row.id),
            name = row.name,
...
addEventHandler("onMarkerHit", resourceRoot, function(player, matchDim)
    ...
    if houseId and houses[houseId] then
        ...
        if previewData[player] and previewData[player].houseId == houseId then
            -- Skip popup if previewing interior, they just hit the exit marker
...
addEventHandler("onPlayerQuit", root, function()
    if previewTimers[source] and isTimer(previewTimers[source]) then
        killTimer(previewTimers[source])
    end
    previewTimers[source] = nil
    previewData[source] = nil
end)
...
addEventHandler("housing:requestEnter", root, function()
    if previewData[client] then
        -- Interior exit marker interaction handled via F-key!
        local data = previewData[client]
        if isTimer(previewTimers[client]) then killTimer(previewTimers[client]) end
        previewTimers[client] = nil
        ...
        end, 1000, 1, client, data)
        previewData[client] = nil
        return
    end
...
addEventHandler("housing:requestToggleLock", root, function()
    if previewData[client] then return end
...
addCommandHandler("previewhouse", function(player)
    ...
    if not house or mType ~= "exterior" then
        outputChatBox("Housing: you must be at an exterior property pickup to preview it.", player, 255, 80, 80)
        return
    end
    ...
    setTimer(function()
        ...
        outputChatBox("Housing: Previewing " .. house.name .. " for 60 seconds. Walk to the door to exit.", player, 120, 255, 255)
        
        previewTimers[player] = setTimer(function(p)
            if not isElement(p) then return end
            if not previewData[p] then return end
            
            fadeCamera(p, false, 1.0)
            local data = previewData[p]
            setTimer(function(pp, d)
                if not isElement(pp) then return end
                setElementInterior(pp, d.int)
                setElementDimension(pp, d.dim)
                setElementPosition(pp, d.x, d.y, d.z)
                setPedRotation(pp, d.rot)
                fadeCamera(pp, true, 1.0)
                outputChatBox("Housing: Preview time expired.", pp, 255, 180, 100)
            end, 1000, 1, p, data)
