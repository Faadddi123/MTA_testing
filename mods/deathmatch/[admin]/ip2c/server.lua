...
local function isResourceRunningByName(resourceName)
	local resource = getResourceFromName(resourceName)
	return resource and getResourceState(resource) == "running"
end

local function isAnyPlayableGamemodeRunning()
	for _, resourceName in ipairs(BOOTSTRAP_GAMEMODES) do
		if isResourceRunningByName(resourceName) then
			return true
		end
	end
	return false
...
local function startResourceIfLoaded(resourceName)
	local resource = getResourceFromName(resourceName)
	if not resource then
		outputDebugString("ip2c bootstrap: missing resource '" .. resourceName .. "'", 2)
		return false
	end
	...
	if state ~= "loaded" then
		outputDebugString("ip2c bootstrap: resource '" .. resourceName .. "' is in state '" .. tostring(state) .. "'", 2)
		return false
	end

	if not startResource(resource, true) then
		outputDebugString("ip2c bootstrap: failed to start '" .. resourceName .. "'", 2)
		return false
	end

	outputDebugString("ip2c bootstrap: started '" .. resourceName .. "'")
	return true
end

local function bootstrapServerGameplay()
	if not isAnyPlayableGamemodeRunning() then
		startResourceIfLoaded("play")
	end

	startResourceIfLoaded("scoreboard")
end

addEventHandler("onResourceStart", resourceRoot,
	function()
		-- Start a playable default mode after the resource index is available.
		setTimer(bootstrapServerGameplay, 1000, 1)
	end
)
